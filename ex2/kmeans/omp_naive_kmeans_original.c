#include <stdio.h>
#include <stdlib.h>
#include <omp.h> // this was not here
#include "kmeans.h"
            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
	...
	#pragma omp parallel for shared(numObjs, numClusters, numCoords, objects, clusters, membership, delta, newClusterSize) private(i, j, index)
        for (i=0; i<numObjs; i++) 
	{
	    ...
	    //do we need to parallel this for?
	    #pragma omp parallel for shared(numCoords, index, newClusters, objects) private(j)
            for (j=0; j<numCoords; j++)
	    {
		#pragma omp atomic
        ...
        #pragma omp parallel for shared(numClusters, numCoords, clusters, newClusterSize) private(i, j) reduce(2)
	for (i=0; i<numClusters; i++) {
            for (j=0; j<numCoords; j++) {

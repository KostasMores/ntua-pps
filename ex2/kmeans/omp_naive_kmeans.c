            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    do {
         ...
         */
	//reduction sto delta
	#pragma omp parallel for shared(numObjs, numClusters, numCoords, objects, clusters, membership, delta, newClusterSize, newClusters) private(i, j, index) default(none)
	for (i=0; i<numObjs; i++) {
	    ...
	    #pragma omp parallel for shared(newClusters, numCoords, objects,i) private(j) 
            for (j=0; j<numCoords; j++)
                /*
	...
	#pragma omp parallel for shared(numClusters, numCoords, clusters, newClusterSize) private(i, j) 
        for (i=0; i<numClusters; i++) {
            for (j=0; j<numCoords; j++) {

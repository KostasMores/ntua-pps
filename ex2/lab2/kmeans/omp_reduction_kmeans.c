            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    for (k=0; k<nthreads; k++)
    {
        ...
        local_newClusters[k] = (typeof(*local_newClusters)) calloc(numClusters * numCoords + (numClusters * numCoords)%(64 / sizeof(**local_newClusters)), sizeof(**local_newClusters));
    }
   ...
   timing = wtime();
	...
	#pragma omp parallel for shared(numObjs, numClusters, numCoords, objects, clusters, membership, newClusterSize, newClusters) private(i,j, index) reduction(+:delta)
        for (i=0; i<numObjs; i++)
        {
	...
	#pragma omp parallel for shared(numClusters, numCoords, newClusterSize, clusters, newClusters) private(i, j) num_threads(nthreads)
        for (i=0; i<numClusters; i++) {
            for (j=0; j<numCoords; j++) {

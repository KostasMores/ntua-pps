                          ...
                          float *devdelta)
{
	...
	/* Maybe only threads with threadID less than numObjs */
    if (tid < numObjs) {
        int   index, i;
        float dist, min_dist;
		...
		/* objectId = tid, clusterId = 0 to initialize min dist to the distance with the first cluster */
						...
						int blockSize)  
{
    ...
    /* TODO: Transpose dims */
    float  *dimObjects = (float*) calloc(numCoords, numObjs, sizeof(float)); //calloc_2d(...) -> [numCoords][numObjs]
	...
	for (i = 0; i < numCoords; i++) {
		for (j = 0; j < numObjs; j++) {
			dimObjects[j*numCoords+i] = objects[j+i*numObjs];	
		}
	}
    ...
    for (i = 0; i < numCoords; i++) {
        for (j = 0; j < numClusters; j++) {
            dimClusters[i][j] = dimObjects[i*numClusters+j];
        }
    }
    ...
    checkCuda(cudaMemcpy(deviceObjects, dimObjects,
    ...
    do {
        ...
        checkCuda(cudaMemcpy(deviceClusters, dimClusters, numCoords*numClusters*sizeof(float), cudaMemcpyHostToDevice));

...
int main(int argc, char ** argv) {
	...
	MPI_Status status;
	MPI_Request req[6], handle_send[2];
	int req_len = 0, send_handles_to_wait = 0;
...
#define T 256
		for (t=0;t<T;t++) {
			...
			if (north != MPI_PROC_NULL){
				...
				MPI_Isend( &u_previous[1][1], 1, row, north, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_current[0][1], 1, row, north, 0, MPI_COMM_WORLD, &req[req_len+1]);
				req_len += 2;
			}

			// Communicate with south
			if (south != MPI_PROC_NULL){
				// MPI_Isend( &u_previous[local[0]][1], 1, row, south, 0, MPI_COMM_WORLD, &req[req_len]);
				MPI_Irecv( &u_previous[local[0]+1][1], 1, row, south, 0, MPI_COMM_WORLD, &req[req_len]);
				req_len++;
			}
			...
			if (east != MPI_PROC_NULL){
				...
				MPI_Irecv( &u_previous[1][local[1]+1], 1, column, east, 0, MPI_COMM_WORLD, &req[req_len]);
				req_len++;
			}
			...
			for (i=i_min;i<i_max;i++)
				for (j=j_min;j<j_max;j++)
					u_current[i][j] = u_previous[i][j] + omega*(u_current[i-1][j]+u_previous[i+1][j]+u_current[i][j-1]+u_previous[i][j+1] - 4*u_previous[i][j])/4.0;
			...
			// Communicate with south
                        if (south != MPI_PROC_NULL){
                                MPI_Isend( &u_current[local[0]][1], 1, row, south, 0, MPI_COMM_WORLD, &handle_send[send_handles_to_wait]);
                                // MPI_Irecv( &u_previous[local[0]+1][1], 1, row, south, 0, MPI_COMM_WORLD, &req[req_len]);
                                send_handles_to_wait++;
                        }

                        // Communicate with east
                        if (east != MPI_PROC_NULL){
                                MPI_Isend( &u_current[1][local[1]], 1,column, east, 0, MPI_COMM_WORLD, &handle_send[send_handles_to_wait]);
                                // MPI_Irecv( &u_previous[1][local[1]+1], 1, column, east, 0, MPI_COMM_WORLD, &req[req_len]);
                                send_handles_to_wait++;
                        }
			
			MPI_Waitall(send_handles_to_wait, handle_send, MPI_STATUSES_IGNORE);
		...
		if (rank==0) {
			printf("Gauss-Seidel SOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,U[global[0]/2][global[1]/2]);

...
//	MPI_Scatter(&U[0][0], local[0]*local[1], &global_block, &u_prev[1][1], local[0]*local[1], &local_block, 0, CART_COMM);
	MPI_Scatterv(&(U[0][0]), scattercounts, scatteroffset, global_block, &(u_current[1][1]), 1, local_block, 0, MPI_COMM_WORLD);
	zero2d(&u_previous[0], local[0]+2, local[1]+2);		
	...
	MPI_Datatype arr_line;
	MPI_Type_contiguous(local[1], MPI_DOUBLE, &dummy);
	MPI_Type_create_resized(dummy,0,sizeof(double),&arr_line);
	MPI_Type_commit(&arr_line);

	MPI_Datatype arr_clmn;
	MPI_Type_vector(local[0], 1, local[1]+2, MPI_DOUBLE, &dummy);
	MPI_Type_create_resized(dummy,0,sizeof(double),&arr_clmn);
	MPI_Type_commit(&arr_clmn);
	...
	//----north
	temp_coords[0] = rank_grid[0]-1;
	temp_coords[1] = rank_grid[1];

	if (temp_coords[0] > -1 && temp_coords[1] > -1) {
		MPI_Cart_rank(CART_COMM, temp_coords, &north);
	}
	else {
		north = -1;
	}
	...
	if (temp_coords[0] > -1 && temp_coords[1] > -1) {
		MPI_Cart_rank(CART_COMM, temp_coords, &south);
	}
	else {
		south = -1;
	}
	...
	temp_coords[0] = rank_grid[0];
	temp_coords[1] = rank_grid[1]+1;

	if (temp_coords[0] > -1 && temp_coords[1] > -1) {
		MPI_Cart_rank(CART_COMM, temp_coords, &east);
	}
	else {
		east = -1;
	}
	...
	if (temp_coords[0] > -1 && temp_coords[1] > -1) {
		MPI_Cart_rank(CART_COMM, temp_coords, &west);
	}
	else {
		west = -1;
	} *\
	...
	*/
	...
	requests_to_wait = 0;
 	...
 	if(north != MPI_PROC_NULL) {
		MPI_Isend(&(u_previous[1][1]), 1, arr_line, north, 0, MPI_COMM_WORLD, &handle[requests_to_wait]);
		MPI_Irecv(&(u_previous[0][1]), 1, arr_line, north, 0, MPI_COMM_WORLD, &handle[requests_to_wait+1]);
		requests_to_wait += 2;
	}
	
        if(east != MPI_PROC_NULL) {
		MPI_Isend(&(u_previous[1][local[1]]), 1, arr_clmn, east, 0, MPI_COMM_WORLD, &handle[requests_to_wait]);
                MPI_Irecv(&(u_previous[1][local[1]+1]), 1, arr_clmn, east, 0, MPI_COMM_WORLD, &handle[requests_to_wait+1]);
                requests_to_wait += 2;
        }

	if(west != MPI_PROC_NULL) {
		MPI_Irecv(&(u_previous[1][1]), 1, arr_clmn, west, 0, MPI_COMM_WORLD, &handle[requests_to_wait]);
                MPI_Irecv(&(u_previous[1][0]), 1, arr_clmn, west, 0, MPI_COMM_WORLD, &handle[requests_to_wait+1]);
                requests_to_wait += 2;
	}

	if(south != MPI_PROC_NULL) {
		MPI_Irecv(&(u_previous[local[0]][1]), 1, arr_line, south, 0, MPI_COMM_WORLD, &handle[requests_to_wait]);
                MPI_Irecv(&(u_previous[local[0]+1][1]), 1, arr_line, south, 0, MPI_COMM_WORLD, &handle[requests_to_wait+1]);
                requests_to_wait += 2;
	}

	MPI_Waitall(requests_to_wait, handle, MPI_STATUSES_IGNORE);
